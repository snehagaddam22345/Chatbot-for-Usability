...
def create_subject(user_id, subject_name, password, content, require, db):
    ...
    try:
        sql = text("""INSERT INTO subjects (subject_name, password, content, require_permission) 
...
def get_subjects(db):
    sql = text("SELECT id, subject_name, require_permission FROM subjects")
...
def get_subject(subject_id, db):
    sql = text("SELECT * FROM subjects WHERE id=:id")
...
def get_rooms(subject_id, db):
    sql = text("SELECT id, room_name FROM rooms WHERE subject_id=:id AND visible=1")
    result = db.session.execute(sql, {"id": subject_id})
...
def is_secret(subject_id, db):
    sql = text("SELECT COUNT(*) FROM subjects WHERE id=:id AND require_permission=1")
    result = db.session.execute(sql, {"id": subject_id})
...
def add_right(user_id, subject_name, db):
    try:
        sql = text("""INSERT INTO subject_rights (user_id, subject_id) 
...
def add_right_id(user_id, subject_id, db):
    try:
        sql = text("INSERT INTO subject_rights(user_id, subject_id) VALUES(:user_id, :subject_id)")
        db.session.execute(
...
def has_right(user_id, subject_id, db):
    if session["admin"]:
        ...
        if is_secret(subject_id, db):
            sql = text("SELECT COUNT(*) FROM subject_rights WHERE user_id = :user_id AND subject_id = :subject_id")
...
def check_password(subject_id, password, db):
    try:
        sql = text("SELECT password, id FROM subjects WHERE id = :subject_id")
